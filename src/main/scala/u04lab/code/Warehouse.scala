...
object Warehouse:
  ...
  private class WarehouseImpl() extends Warehouse:
    ...
    override def searchItems(tag: String): List[Item] = filter(items)(item => !isEmpty(find(item.tags)(t => t == tag)))
    ...
    override def contains(itemCode: Int): Boolean = !isEmpty(find(items)(item => item.code == itemCode))
